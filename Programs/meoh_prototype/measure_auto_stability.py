...
PRESSURE_ANALYSIS = -26.25
PRESSURE_REGENERATION = -150

TIME_MEASUREMENT = 10
TIME_ANALYSIS = 90
TIME_REGENERATION = 600
...
def setup_file_logger():
    ...
    filename = 'log_s_' + datetime.now().strftime('%y%m%d') + '_' + file_add
...
#     FastPressure
    if FASTP == 1:
        filename = 'log_s_' + datetime.now().strftime('%y%m%d') + '_' + 'pressure_fast'
...
def auto_measurement():
    if HEADS == 1:
        ...
        stability_sampling(1)
    elif STAB_TEST == 1:
        stability_sampling(2)
    else:
...
if __name__ == '__main__':
    ...
    layout = [[SG.Text('Pressure'), SG.In(size=(10, 1), key='-PRESSURE-'), SG.Button('SET')],
              ...
              [SG.Checkbox('Headspace', default=False, key='H'), SG.Checkbox('Vacuum', default=False,key='V')],
              [SG.Checkbox('Stop detection', default=False,key='S'), SG.Checkbox('Reapplying', key='R')],
              [SG.Checkbox('Pressure hold', default=False,key='P'), SG.Checkbox('Vacuum Test mode', key='T')],
              [SG.Checkbox('Stability', default=True,key='Stab'), SG.Checkbox('Stability Test', default=False,key='StabTest')]]
